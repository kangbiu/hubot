...
if process.env.DEVELOPER_CHAT_ROOM
  ROOM = process.env.DEVELOPER_CHAT_ROOM
  console.log "DEVELOPER_CHAT_ROOM defined in Env Variable; Room id:" + ROOM
else
  ROOM = "53157_developers@conf.hipchat.com"
  console.log "DEVELOPER_CHAT_ROOM ＊NOT＊ defined in Env Variable; Use Default Room id:" + ROOM
...
module.exports = (robot) ->
  ...
  robot.hear PRODUCTION_DEPLOYMENT_TEXT, (msg) ->
    console.log "Hubot - Hear Commit Message - " + PRODUCTION_DEPLOYMENT_TEXT
    redis.set ALREADY_COMMIT, true
    lastPushTime = getCurrentTime()
    redis.set LAST_PUSH_TIME, lastPushTime  
  ...
  robot.respond LAST_PUSH_TEXT, (msg) ->
    redis.get LAST_PUSH_TIME, (err, replies) ->
      if err
        msg.send "Sorry! ERROR occur in redis read"
      
      lastPushDateFromDB = new Date(replies)
      lastPushDateFromDB.setTimezone TIMEZONE
      lastPushDate = lastPushDateFromDB.getDate()

      nowTime = new Date()
      nowTime.setTimezone TIMEZONE
      nowDate = nowTime.getDate()

      dateDiff = nowDate - lastPushDate

      console.log "LAST PUSH TIME: CurrentTime: " + nowTime + "; Last Update Time: " + lastPushDateFromDB

      if dateDiff == 0
        msg.send "Last push Time: Today "+  lastPushDateFromDB.getHours() + ":"+lastPushDateFromDB.getMinutes() + " HK TIME"
      else
        msg.send "Last Time: " + lastPushDateFromDB 
...
getCurrentTime = () ->
  ...
  time = date.toString("YY-MM-dd h:mm:ss")
  time
